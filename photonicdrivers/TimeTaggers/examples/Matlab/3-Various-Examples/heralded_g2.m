...
pulse_freq = 10e6;  % Hz
laser_sync_ch = 4;
binwidth = int64(1 / pulse_freq * 1e12);
bins = 100;
meas_time = 1e12;  % ps
...
s1i = TTCoincidence(sm_tagger, [idler_ch, signal_1_ch], 100, ...
                   TTCoincidenceTimestamp.ListedFirst);
corr_ssi = TTCorrelation(sm_tagger, s1i.getChannel(), signal_2_ch, binwidth, bins);
corr_s2i = TTCorrelation(sm_tagger, idler_ch, signal_2_ch, binwidth, bins);
cnr = TTCountrate(sm_tagger, [s1i.getChannel(), idler_ch]);

sm.startFor(meas_time);
sm.waitUntilFinished();

Nssi_data = double(corr_ssi.getData());
Nssi_lag_times = corr_ssi.getIndex();
Ns2i_data = double(corr_s2i.getData());
rates = cnr.getData();
Rs1i = rates(1);
Ri = rates(2);
...
g2_heralded = Nssi_data * Ri / (Ns2i_data * Rs1i);

% Plotting
figure()
plot(Nssi_lag_times, g2_heralded);
ylabel('heralded g2');
xlabel('lag time (ps)');
