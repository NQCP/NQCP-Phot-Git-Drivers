...
//  LDA_Demo.cpp
//
//	This program is an example of how to use the Vaunix Lab Brick Attenuator DLL from a Windows application
//
//
//	7/2016		modified to show an example of using the new HiRes functions
//	10/31/2016	modified to show an example of using the Quad Attenuator functions
//  1/6/2018	modified to support simple multi-channel attenuator operations
//	6/11/2019	modified to show how to launch multiple ramps nearly simultaneously
//  5/25/20		modified to report ramp and profile status, and the larger number of channels present in expandable LDA devices
//	2/11/22		updated to implement more features and remove the obsolete RF On/Off status and command
//  10/28/25	changed name from LDA64Test.cpp to LDA_Demo.cpp. Changed user input from .25 db units to db units for consistency with other demo programs
//
...
static float AStop = 63;				// default atten stop, for most devices this is 63 db so we use that.
...
static float AStep = 0.5;				// default step size is .5db, some LDA's have larger and some have smaller minimum steps
...
void PrintHelp()
{
	...
	printf("  -g m		Start a ramp, m is the mode:\n");
...
bool ParseCommandLine(int argc, _TCHAR *argv[])
{
	...
	enum {
		...
		wantAtten, wantSweep, wantGetParam, wantDevID, wantDevRange,
	...
	if (NumDevices > 0)	// do we have a device?
	{
		for (j = 0; j < NumDevices; j++) {
			...
			if (!gbQuietMode && gbWantReadSettings) {
				...
				for (i = 0; i < NumChannels; i++)
				{
					...
					if (fnLDA_GetFeatures(MyDevices[j]) > 0)
					{
						itemp = fnLDA_GetDeviceStatus(MyDevices[j]);
						if (itemp >= 0)
							printf("Device Status = %d\n", itemp&DEVSTATUS_MASK);
						else
							CheckAPISet(itemp);
					}
		...
		for (iDev = DevNum; iDev < DevNum + DevRange; iDev++) {
			...
			if (gbWantSetAStart) {
				...
				// using the HiRes API function with .05db units
				ScaledStart = (int)(AStart * 20);

				if (!gbQuietMode) printf("Setting the ramp start for device %d to %.2f db\n", MyDevices[iDev], AStart);
				itemp = fnLDA_SetRampStartHR(MyDevices[iDev], ScaledStart);
				CheckAPISet(itemp);
			}
		...
		for (iDev = DevNum; iDev < DevNum + DevRange; iDev++)
		{
			if (gbWantStartSweep)
			{
				...
				else
				{
					...
					// -- as an example, the StartRamp multi-channel call listed below --
					//	  will start a single, upwards ramp on the first 4 channels
					//	  the final argument deferred=false indicates that the ramps should start immediately
					//	  multi-channel ramps and profiles are only supported on some LDA devices
					//itemp = fnLDA_StartRampMC(MyDevices[iDev], 0x01, 0x0F, false);
