...
class santec_MPM220_driver(Identifiable, Connectable):
    def __init__(
        self, address: str = "GPIB0::16::INSTR", timeout: int = 2
    ) -> None:
    ...
    def connect(self) -> None:
        ...
        self.instrument.timeout = self.timeout * 5000  # convert s → ms
    ...
    def read_bytes(self, num_bytes: int):
        return self.instrument.read_bytes(num_bytes)
    def set_measurement_mode(self, mode: str):
        ...
        if mode not in ["CONST1", "SWEEP1", "CONST2", "SWEEP2", "FREE-RUN"]:
            raise ValueError(
                "Invalid mode. Supported modes: CONST1, SWEEP1, CONST2, SWEEP2, FREE-RUN"
            )
    ...
    def set_power_mode(self, value):
        ...
        if value not in [0, 1]:
            raise ValueError(
                "Invalid value. It must be 0 (Manual range) or 1 (Auto range)."
            )
    ...
    def set_average_time(self, time):
        ...
        if not (0.01 <= time <= 10000.00):
            raise ValueError(
                "Invalid time value. It must be between 0.01 and 10000.00 ms."
            )
    ...
    def set_power_unit(self, value):
        ...
        if value not in [0, 1]:
            raise ValueError(
                "Invalid value. It must be 0 (for dBm/dBmA) or 1 (for mW/mA)."
            )
    ...
    def get_logging_status(self):
        ...
        """
        status, count = self.connection.instrument.query("STAT?").split(",")
    ...
    def get_logging_data_point(self):
        ...
        response = self.connection.instrument.query("LOGN?")
        return int(response)
    ...
    def get_logging_data(self, module_no: int, channel_no: int):
        ...
        try:
            ...
            return self.connection._query_binary_values(
                f"LOGG? {module_no},{channel_no}", data_points=expected_size
            )
    ...
    def query(self, command: str) -> str:
        ...
        return self.instrument.query(command)  # type: ignore
